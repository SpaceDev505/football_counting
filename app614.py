...
def gen_frams():
    ...
    #                                 index = j
    #                                 if(j == 2 or j == 3):
    #                                     if( left_3d_knee_angle < 80 or right_3d_knee_angle < 80) and (y+ radius < max(pos_y[2], pos_y[3])):
    #                                         print(' errrrrorrrrrrrrrrrrr', y-radius, max(pos_y[2], pos_y[3]) )
    #                                         print('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
    #                                         if(left_3d_knee_angle < 80):
    #                                             pose_count[2] += 1
    #                                         elif(right_3d_knee_angle < 80):
    #                                             pose_count[3] += 1
    #                                     else:
    #                                         if(diff[0]< diff[1]):
    #                                             pose_count[0] += 1
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ret, frame = cap.read()
            # cap.set(cv2.CAP_PROP_POS_FRAMES, index_rate * frame_rate) 
            if frame is not None:
                frame_ = rescale_frame(frame, percent=100)
            i += 1
            if i%3:
            ...
            try:
                ...
                right_ankle = [landmarks[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value].y]
                left_ankle = [landmarks[mp_pose.PoseLandmark.LEFT_FOOT_INDEX.value].x,landmarks[mp_pose.PoseLandmark.LEFT_FOOT_INDEX.value].y]
                right_ank = [landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE.value].x,landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE.value].y]
                left_ank = [landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].x,landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].y]
            ...
            for i , seg_cnt in enumerate(segs):
                if dets[i][5] == 32:
                    ...
                    radius = int(radius)
                    cv2.circle(image,center,radius,(0,255,0),2)
                    # cv2.circle(image, (int(x), int(y)),3 (0, 255, 255), 2)
            print('Positio', int(y), int(pre_y))
        
            pos_x = [left_ankle[0]*width, right_ankle[0]*width, left_knee[0]*width, right_knee[0]*width, left_shoulder[0]*width, right_shoulder[0]*width, head[0]*width]
            pos_y = [left_ankle[1]*height, right_ankle[1]*height, left_knee[1]*height, right_knee[1]*height, left_shoulder[1]*height, right_shoulder[1]*height, head[1]*height]

            right_knee_angle = calculate_angle(right_hip, right_knee, right_ank)
            left_knee_angle = calculate_angle(left_hip, left_knee, left_ank)
            print('angle1111111', right_knee_angle, left_knee_angle)
            if(y!=0 and pre_y !=0):
                if(y-pre_y !=0):
                    if((y- pre_y)>2):
                        direct = "Down"
